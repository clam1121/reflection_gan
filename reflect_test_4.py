...
class TokenLogger(BaseCallbackHandler):
    def on_llm_start(self, serialized, prompts, **kwargs):
        self.input_tokens = sum(len(prompt.split()) for prompt in prompts)  # 近似统计输入token
        logger.info(f"Input tokens: {self.input_tokens}")
        
    def on_llm_end(self, response, **kwargs):
        output = response.generations[0][0].text
        self.output_tokens = len(output.split())  # 近似统计输出token
        logger.info(f"Output tokens: {self.output_tokens}")

def initialize_logger(log_dir='//home/guzhouhong/cxz/reflect/reflection/llm_output_log', log_file='output.log', max_bytes=10**6, backup_count=5):
    """
...
global llm_name
llm_name = "qwen2.5:7b"
llm = Ollama(model = llm_name, callbacks=[TokenLogger()])

def extract_array_from_response(response):
    try:
        # 使用正则匹配键值对格式
        if "#### Fake_event" in response:
            temp_1 = response.split("#### Fake_event:")[-1]
            res = temp_1.split("#### Explain:")
            if (len(res) < 2):
                logger.info("无法解析响应格式: " + response)
                return ["none", "none"]
            return res    
        elif "#### Result" in response:
            temp_1 = response.split("#### Result:")[-1]
            res = temp_1.split("#### Explain:")
            if (len(res) < 2):
                logger.info("无法解析响应格式: " + response)
                return ["none", "none"]
            ...
            temp_1 = response.split("#### Reflection:")[-1]
            res = temp_1.split("#### New_strategy:")
            if (len(res) < 2):
                logger.info("无法解析响应格式: " + response)
                return ["none", "none"]
            return res  
        else:
            print("无法解析响应格式: " + response)
            logger.info("无法解析响应格式: " + response)
            return ["none", "none"]
...
# 2. 模型构建
# 构建生成器
def generate_fake_news(real_news, fake_strategies="空"):
...
# 构建反思器
def reflect_on_detection(news_with_fake_explanations, detection_strategies):
    # 此处的news应该是虚假新闻和解释的列表，
    prompt = ChatPromptTemplate.from_messages(
        [
    ...
    usr_prompt = reflect_on_detection_usr_prompt.format(news_with_fake_explanations = news_with_fake_explanations,detection_strategies=detection_strategies)
    # response = chat(sys_prompt,usr_prompt)

    chain_1 = prompt|llm
    response = chain_1.invoke({'news_with_fake_explanations':news_with_fake_explanations,'detection_strategies':detection_strategies})
...
# 5. 模型训练与评估
def train_llm_gan( train_real, train_fake, val_real, val_fake, Correct_distance, Incorrect_distance, num_epochs, leng_fake_strategies=5, leng_detection_strategies=20):
    ...
    for epoch in range(num_epochs):
        ...
        fake_news_with_fake_explanations = []
        Correct_step = 0
        Incorrect_step = 0
        all_news = 0
        for real_news in tqdm(train_real, desc=f"Training Epoch {epoch+1}", unit="sample"):
            ...
            if detection_result == "none":
                continue
            # 根据检测结果更新生成器和检测器策略
            if "Fake_news" in detection_result:
                # 更新生成器策略
            ...
            if "True_news" in detection_result:
                # 更新检测器策略
                Incorrect_step += 1
                fake_news_with_fake_explanations.append(f"虚假事件{Incorrect_step}:{fake_news},该事件的解释:{fake_explanations}")
                    # print(f"detection_strategies:{detection_strategies}")
            all_news += 1
        if len(fake_news_with_fake_explanations) > 5:
            temp_fake_news_with_explain = '\n'.join(fake_news_with_fake_explanations[:5])
        else:
            temp_fake_news_with_explain = '\n'.join(fake_news_with_fake_explanations)
        print("*"*30 + "len(temp_fake_news_with_explain)")
        print(len(temp_fake_news_with_explain))
        new_detection_strategies = reflect_on_detection(temp_fake_news_with_explain, detection_strategies)
        detection_strategies_list.append(new_detection_strategies[-1])
        detection_strategies_queue.enqueue(new_detection_strategies[-1])
...
if __name__ == "__main__":
    ...
    res_1, res_2, res_3 = train_llm_gan(train_real, 
                                        train_fake, 
                                        val_real, 
                                        val_fake, 
                                        Correct_distance, 
    ...
    write_json_to_file(res_1, f"output_file/output_v4/{llm_name}_Incorrect_distance{Incorrect_distance}_epoch_{num_epochs}_fake_strategies_without_reflect")
    write_json_to_file(res_2, f"output_file/output_v4/{llm_name}_Incorrect_distance{Incorrect_distance}_epoch_{num_epochs}_detection_strategies_without_reflect")
    write_json_to_file(res_3, f"output_file/output_v4/{llm_name}_Incorrect_distance{Incorrect_distance}_epoch_{num_epochs}_accuracy_list_without_reflect")
